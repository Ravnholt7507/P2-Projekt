...
function medicalEquip(HasEquipBool){
 // read array with data and evaluate
 if(HasEquipBool == 1){
    console.log('There is enough equipment, checking if there is staff available');
    staffAvailable(StaffBool);
   //console.log('There is enough equipment, calling BedAllocation function'); 
   //call BedAllocation
}else if(HasEquipBool == 0){
    console.log('Not enough equipment available, checking region');
    CheckRegion();
}else {
    console.error('Something went wrong, unable to determine value of EquipmentBool.');
    // Restart or shut down program?
}
}
...
function staffAvailable(StaffBool){
  // read array with data and evaluate
if(StaffBool == 1){
    console.log('Enough staff available, checking distance.');
    checkDistance
} 
else if(StaffBool == 0){
    console.log('Not enough staff available, checking region');
    CheckRegion();
}
else{
    console.error('Something went wrong, unable to determine value of StaffBool.');
    // Restart or shut down program?
}
}
